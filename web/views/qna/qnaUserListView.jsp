...
<div style="text-align:center">
	...
	<% }else if(searchTitle != null){ %>
		<a href="/math/quslist?page=1&title=<%=searchTitle%>&qOption=<%=qOption%>">[맨처음]</a>
	<% }else{ %>
		<a href="/math/quslist?page=1">[맨처음]</a>
	<%} if((currentPage - 5) <= startPage && (endPage - 5) >= 1){ %>
		<a href="/math/quslist?page=<%= (startPage - 5) + 4   %>&title=<%=searchTitle%>&qOption=<%=qOption%>">[이전]</a>
	<% }else{%>
		[이전]
	<%} for(int p = startPage; p <= endPage; p++){ 
			...
			<%}else if(searchTitle != null && qOption != null){ %>
				<a href="/math/quslist?page=<%=p%>&title=<%=searchTitle%>&qOption=<%=qOption%>"><%=p %></a>
				<%}else{ %>
				<a href="/math/quslist?page=<%=p%>"><%=p %></a>
	<% }} %>
	<% if((startPage + 5) <= maxPage && (currentPage + 5) >= startPage){ %>
		<a href="/math/quslist?page=<%=startPage+5%>&title=<%=searchTitle %>&qOption=<%=qOption%>">[다음]</a>
	<%}else{ %>
		[다음]
	<%} %>
	<% if(currentPage >= maxPage){ %>
		[마지막]
	<% }else if(searchTitle != null){ %>
		<a href="/math/quslist?page=<%=maxPage%>&title=<%=searchTitle%>&qOption=<%=qOption%>">[마지막]</a>
	<% }else{ %>
		<a href="/math/quslist?page=<%=maxPage%>">[마지막]</a>
	<%} %>
</div>
<div>
<form action="/math/qsulist" method="post">
<select name="qOption">
...
<button onclick="myQnaList();">내 글보기</button>
<script type="text/javascript">
function questionWrite(){
	location.href="/math/views/qna/questionWriteView.jsp";
}
function myQnaList(){
	location.href = "/math/qmuslist?userId=<%=userId%>";
}
</script>
</body>
</html>